...
function planLabel(planCode: string): string { switch (planCode) { case "light": return "轻享版"; case "support": return "支持版"; case "admin": return "管理员套餐"; default: return "免费版"; } }
    ...
    <section style={{ display: "grid", gap: 18 }}>
      <header className="panel" style={{ padding: 28, display: "grid", gap: 14 }}>
        ...
        <nav aria-label="管理后台页面" style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <a className="button-secondary" href="/admin/contest">创作比赛管理</a>
        </nav>
      ...
      {activeSection === "users" ? (<section className="panel" style={{ padding: 24, display: "grid", gap: 16 }} aria-labelledby="admin-users-title"><h2 id="admin-users-title" style={{ margin: 0 }}>{text.users}</h2>{users.length === 0 ? <p style={{ margin: 0, color: "var(--muted)" }}>{text.noUsers}</p> : null}<div style={tableWrapStyle} role="region" aria-label={`${text.users}${text.tableRegion}`}><table style={tableStyle}><thead><tr><th style={cellStyle}>用户名</th><th style={cellStyle}>邮箱</th><th style={cellStyle}>{text.role}</th><th style={cellStyle}>{text.plan}</th><th style={cellStyle}>作品数</th><th style={cellStyle}>{text.createdAt}</th><th style={cellStyle}>{text.actions}</th></tr></thead><tbody>{users.map((user) => (<tr key={user.id}><td style={cellStyle}>{user.username || "未设置"}</td><td style={cellStyle}>{user.email}</td><td style={cellStyle}><label className="field" style={{ gap: 4 }}><span className="sr-only">{user.email} 的角色</span><select value={editingUsers[user.id]?.role ?? user.role} onChange={(event) => setEditingUsers((current) => ({ ...current, [user.id]: { role: event.target.value, planCode: current[user.id]?.planCode ?? user.planCode } }))}><option value="user">普通用户</option><option value="admin">管理员</option><option value="super_admin">超级管理员</option></select></label><span style={{ color: "var(--muted)", fontSize: "0.9rem" }}>当前：{roleLabel(user.role)}</span></td><td style={cellStyle}><label className="field" style={{ gap: 4 }}><span className="sr-only">{user.email} 的套餐</span><select value={editingUsers[user.id]?.planCode ?? user.planCode} onChange={(event) => setEditingUsers((current) => ({ ...current, [user.id]: { role: current[user.id]?.role ?? user.role, planCode: event.target.value } }))}><option value="free">免费版</option><option value="light">轻享版</option><option value="support">支持版</option><option value="admin">管理员套餐</option></select></label><span style={{ color: "var(--muted)", fontSize: "0.9rem" }}>当前：{planLabel(user.planCode)}</span></td><td style={cellStyle}>{user.projectCount}</td><td style={cellStyle}>{formatDate(user.createdAt)}</td><td style={cellStyle}><button className="button-secondary" type="button" onClick={() => saveUserSettings(user.id)}>{text.saveUser}</button></td></tr>))}</tbody></table></div></section>) : null}
