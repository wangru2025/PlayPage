...
public sealed class MainActivity : Activity
{
    ...
    private void ShowProjectActions(int position)
    {
        ...
        var actions = new[] { "打开作品", "查看详情", "切换广场显示", "上传新版本", "申请修复", "查看修复申请", "AI 圆桌", "申请独立网址", "查看独立网址", "查看历史版本", "统计数据" };
    ...
    protected override async void OnActivityResult(int requestCode, Result resultCode, Intent? data)
    {
        ...
        if (requestCode != PickReleaseFileRequest || resultCode != Result.Ok || data?.Data == null || _pendingUploadProject == null) return;
        try
        {
            var uri = data.Data;
